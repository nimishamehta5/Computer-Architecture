...
uns64   dram_access_sim_rowbuf(DRAM *dram,Addr lineaddr, Flag is_dram_write){
  uns64 delay=0;
    ...
    uns64 bank_id,row_in_bank,num_lines,left_part;

    num_lines=(ROWBUF_SIZE/CACHE_LINESIZE);
    //row_in_bank=(lineaddr % num_lines);
    left_part=(lineaddr / num_lines);
    bank_id=(left_part % DRAM_BANKS);
    row_in_bank=(left_part / DRAM_BANKS);

    if(dram->perbank_row_buf[bank_id].valid==0)
    {
      dram->perbank_row_buf[bank_id].valid=1;
      dram->perbank_row_buf[bank_id].rowid=row_in_bank;
      delay=DRAM_T_ACT+DRAM_T_CAS+DRAM_T_BUS;
    }
    else if((dram->perbank_row_buf[bank_id].valid) && (dram->perbank_row_buf[bank_id].rowid == row_in_bank))
    {
      delay=DRAM_T_CAS+DRAM_T_BUS;
    }
    else if((dram->perbank_row_buf[bank_id].valid) && (dram->perbank_row_buf[bank_id].rowid != row_in_bank))  //conflict
    {
      dram->perbank_row_buf[bank_id].rowid=row_in_bank;
      delay=DRAM_T_PRE+DRAM_T_ACT+DRAM_T_BUS+DRAM_T_CAS;
    }
